...
export default function CreateServerGroup({ buttonText }: ImportCsvModalProps) {
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        ...
        for (const row of data as any[]) {
          const { siteName, serverGroupName, ...body } = row;
          ...
          if (!siteName || !serverGroupName) {
            setError('CSV row missing siteName or serverGroupName');
            return;
          }

          const url = `/api/createServerGroup?siteName=${encodeURIComponent(siteName)}&serverGroupName=${encodeURIComponent(serverGroupName)}`;
